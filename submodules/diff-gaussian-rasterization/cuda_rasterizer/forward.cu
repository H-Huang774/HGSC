...
calCUDA(
	...
	const float* __restrict__ bg_color,
	float* __restrict__ out_color,
	float* __restrict__ significance_opacity
	)
{
	// Identify current tile and associated min/max pixel range.
	auto block = cg::this_thread_block();
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			significance_opacity[collected_id[j]] += alpha * T;
			...
			// atomicAdd(&(significance_opacity[collected_id[j]]), alpha * T);
...
void FORWARD::calc(
	...
	const float* bg_color,
	float* out_color,
	float* significance_opacity)
{
	calCUDA<NUM_CHANNELS> << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		means2D,
